...
class RobustDeepfakeDetector:
    def __init__(self, model_path: str = None):
        """
        Initialize the Robust Deepfake Detector
    ...
    def create_model(self):
        ...
        print("Creating custom CNN model for deepfake detection...")
    ...
    def train_quick_model(self):
        ...
        print("Training custom model with sample data...")
        
        # Create sample data
        from data_preparation import DataPreparator
        preparator = DataPreparator()
        preparator.create_sample_dataset(num_real=300, num_fake=300)
    ...
    def analyze_image_artifacts(self, image: np.ndarray) -> Dict:
        ...
        jpeg_artifacts = self.detect_jpeg_artifacts(gray)
        
        # 6. Noise analysis
        noise_level = self.analyze_noise_patterns(gray)
        
        # Combine features into a fake probability
        fake_score = 0.0
        
        # Edge density contribution (0-0.2)
        if edge_density > 0.1:
            fake_score += min(0.2, edge_density * 1.5)
        
        # High frequency contribution (0-0.3)
        if high_freq_score > 8:
            fake_score += min(0.3, (high_freq_score - 8) * 0.04)
        
        # Color variance contribution (0-0.15)
        if color_variance > 1000:
            fake_score += min(0.15, (color_variance - 1000) / 8000)
        
        # LBP entropy contribution (0-0.1)
        if lbp_entropy > 2.5:
            fake_score += min(0.1, (lbp_entropy - 2.5) * 0.08)
        
        # JPEG artifacts contribution (0-0.15)
        fake_score += min(0.15, jpeg_artifacts)
        
        # Noise pattern contribution (0-0.1)
        fake_score += min(0.1, noise_level)
    ...
    def detect_jpeg_artifacts(self, gray_image):
        ...
        for i in range(0, h - block_size, block_size):
            for j in range(0, w - block_size, block_size):
                ...
                if np.std(block) < 10:  # Low variance suggests compression
                    artifacts += 0.1
        
        return min(1.0, artifacts / ((h // block_size) * (w // block_size)))
    ...
    def analyze_noise_patterns(self, gray_image):
        ...
        noise_uniformity = np.std(local_variance)
        return min(1.0, noise_uniformity / 1000)
    ...
    def detect_image(self, image_path: str) -> Dict:
        ...
        try:
            ...
            if hasattr(self, '_is_trained') and self._is_trained:
                combined_confidence = 0.8 * ai_confidence + 0.2 * artifact_confidence
            else:
    ...
    def detect_video(self, video_path: str, sample_frames: int = 30) -> Dict:
        ...
        try:
            ...
            # Combine predictions
            if hasattr(self, '_is_trained') and self._is_trained:
                combined_confidence = 0.8 * avg_ai_confidence + 0.2 * avg_artifact_confidence
            else:
            ...
            for i, (frame_idx, ai_pred, artifact_pred) in enumerate(zip(frame_indices, ai_predictions, artifact_predictions)):
                frame_combined = 0.8 * ai_pred + 0.2 * artifact_pred if hasattr(self, '_is_trained') and self._is_trained else 0.3 * ai_pred + 0.7 * artifact_pred
